...
export function AskOverlay() {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-[var(--surface-primary)] flex flex-col animate-slide-up">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-center py-8">
            ...
            <h2 className="font-bold text-[var(--text-primary)] mb-2">How can I help?</h2>
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {['How should croissants be displayed?', 'Coffee machine cleaning', 'Spill procedures'].map((suggestion) => (
                <button
                  ...
                  className="text-xs px-3 py-1.5 rounded-full bg-[var(--surface-secondary)] text-[var(--text-secondary)] hover:bg-[var(--surface-tertiary)] transition-colors"
                >
                  {suggestion}
                </button>
              ))}
        ...
        {isTyping && (
          <div className="flex justify-start">
            <div className="bg-[var(--surface-secondary)] rounded-2xl rounded-bl-md px-4 py-3">
      ...
      {/* Input */}
      <div className="flex-shrink-0 p-4 border-t border-[var(--border-primary)] bg-[var(--surface-primary)] safe-bottom">
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 rounded-full bg-[var(--surface-secondary)] text-[var(--text-primary)] placeholder-[var(--text-muted)] text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:ring-offset-2"
          />
          <button
            type="submit"
            disabled={!input.trim() || isTyping}
            className="w-12 h-12 rounded-full bg-[var(--color-primary)] text-white flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
          >
